...
Consumes low-stock alerts from Kafka and simulates notifying a procurement
team. Exists to demonstrate Kafka fan-out: the same event, consumed
independently by multiple services in different consumer groups.
...
## Run
    mvn spring-boot:run
Runs on port 8085 (from `config-repo/notification-service.yml`). Requires
